...
plot_best_methods = function(n, eps) {
    ...
    if (n == 2500) {
        for (k in 1:20) {
            file_name = paste0("output/simulation_normalnoise/kng_n", n, "_eps", eps, "_", k, ".Rdata")
            load(file_name)
            for (i in 1:25) {
                tmp = t(oper[[i]])
                for (j in 1:8) {
        ...
        for (k in 1:20) {
            file_name = paste0("output/simulation_normalnoise/opm_sgd_n", n, "_eps", eps, "_", k, ".Rdata")
            load(file_name)
            for (i in 1:25) {
                tmp = t(oper[[i]])
                for (j in 1:12) {
        ...
        for (k in 1:20) {
            file_name = paste0("output/simulation_normalnoise/agd_np_n", n, "_eps", eps, "_", k, ".Rdata")
            load(file_name)
            for (i in 1:25) {
                tmp = t(oper[[i]])
                for (j in 1:7) {
...
save_file = paste0('./plot/', save_date, '_compare_all_normal.png')
...
plot_all_methods = function(method, n, eps) {
    ...
    if (method == 'kng') {ylim = c(-1.5, 1)} 
    else if (method == 'opm') {ylim = c(-1.5, 1)}
    else if (method == 'sgd') {ylim = c(-1.5, 3)}
...
for (method in methods) {
    ...
    save_date = paste0(substring(save_date, 3, 4), substring(save_date, 6, 7),
                       substring(save_date, 9, 10))
    save_file = paste0('./plot/', save_date, '_compare_all_', method, '_normal.png')
