...
# The output is stored in folder output/simulation_heteroscedastic and it can be 
# plotted using file plot_simulation_heteroscedastic.R.
# Last edited: 3/20/24
...
oper = foreach(i=start:end, .combine=rbind, .multicombine=TRUE, #change
               .init=list()) %dopar% {
                   ...
                   out = dp_qr(x = x, y = y, theta = theta, epsilon = 1, Cx = 2,
                               taus = c(0.05, 0.25, 0.5, 0.75, 0.95, 0.99)) #change
               }

stopCluster(workers)

save(oper, file = paste0("./output/simulation_heteroscedastic/agd_np_n2500_eps1_", iter, ".Rdata"))
