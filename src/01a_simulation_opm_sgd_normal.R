...
sgd_convsm = function(theta, x, y, tau, h, epsilon = 1, Cx, eta,
    ...
    for(i in 1:(n^2-1)) {
        ...
        theta_all[i+1, ] = theta_all[i, ] - 1/sqrt(i)*(grad + w) #eta?
    }
...
oper = foreach(i=start:end, .combine=rbind, .multicombine=TRUE, #change
               .init=list()) %dopar% {
                   ...
                   out = dp_qr3(x = x, y = y, theta = theta, epsilon = 1, Cx = 10,
                                taus = c(0.05, 0.25, 0.5, 0.75, 0.95, 0.99)) #change
               }

stopCluster(workers)

save(oper, file = paste0("./output/simulation_normalnoise/opm_sgd_n2500_eps1_", iter, ".Rdata"))
