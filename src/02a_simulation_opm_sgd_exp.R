...
# in the simulation setting with exponential noise. For each replication, we estimate
...
# The other two file has name simulation_[method]_exp.R.
# The output is stored in folder output/simulation_expnoise and it can be plotted 
# using file plot_simulation_exp.R.
# Last edited: 2/8/24
...
dp_qr3 = function(x, y, theta, taus, epsilon, eta = 0.01, Cx = 1) {
    ...
    for (t in 1:length(taus)) {
        ...
        true_theta = c(theta)
        true_theta[1] = true_theta[1] + log(1-tau)/(-0.1) #0.1 is the rate
        
        out_time[t, 1] = system.time({tmp = optim(c(0, 0, 0), opm_convsm, x = x, y = y,  
                                                  Cx = Cx, tau = tau, h = 0.5, 
                                                  epsilon = epsilon, kernel = 'gaussian')$par})[3]
        out_par[[1]] = rbind(out_par[[1]], tmp)
        
        out_time[t, 2] = system.time({tmp = optim(c(0, 0, 0), opm_convsm, x = x, y = y,  
                                                  Cx = Cx, tau = tau, h = 3, 
                                                  epsilon = epsilon, kernel = 'gaussian')$par})[3]
        out_par[[2]] = rbind(out_par[[2]], tmp)
        
        out_time[t, 3] = system.time({tmp = optim(c(0, 0, 0), opm_convsm, x = x, y = y,  
                                                  Cx = Cx, tau = tau, h = 0.5, 
                                                  epsilon = epsilon, kernel = 'logistic')$par})[3]
        out_par[[3]] = rbind(out_par[[3]], tmp)
        
        out_time[t, 4] = system.time({tmp = optim(c(0, 0, 0), opm_convsm, x = x, y = y,  
                                                  Cx = Cx, tau = tau, h = 3, 
                                                  epsilon = epsilon, kernel = 'logistic')$par})[3]
        out_par[[4]] = rbind(out_par[[4]], tmp)
        
        out_time[t, 5] = system.time({tmp = optim(c(0, 0, 0), opm_convsm, x = x, y = y,  
                                                  Cx = Cx, tau = tau, h = 0.5, 
                                                  epsilon = epsilon, kernel = 'uniform')$par})[3]
        out_par[[5]] = rbind(out_par[[5]], tmp)
        
        out_time[t, 6] = system.time({tmp = optim(c(0, 0, 0), opm_convsm, x = x, y = y,  
                                                  Cx = Cx, tau = tau, h = 3, 
                                                  epsilon = epsilon, kernel = 'uniform')$par})[3]
        out_par[[6]] = rbind(out_par[[6]], tmp)
        
        out_time[t, 7] = system.time({tmp = sgd_convsm(theta = c(0, 0, 0), x = x, y = y, 
                                                       tau = tau, h = 0.5, epsilon = epsilon, 
                                                       Cx = Cx, eta = eta, kernel = 'gaussian')})[3]
        out_par[[7]] = rbind(out_par[[7]], tmp)
        
        out_time[t, 8] = system.time({tmp = sgd_convsm(theta = c(0, 0, 0), x = x, y = y, 
                                                       tau = tau, h = 3, epsilon = epsilon, 
                                                       Cx = Cx, eta = eta, kernel = 'gaussian')})[3]
        out_par[[8]] = rbind(out_par[[8]], tmp)
        
        out_time[t, 9] = system.time({tmp = sgd_convsm(theta = c(0, 0, 0), x = x, y = y, 
                                                       tau = tau, h = 0.5, epsilon = epsilon, 
                                                       Cx = Cx, eta = eta, kernel = 'logistic')})[3]
        out_par[[9]] = rbind(out_par[[9]], tmp)
        
        out_time[t, 10] = system.time({tmp = sgd_convsm(theta = c(0, 0, 0), x = x, y = y, 
                                                       tau = tau, h = 3, epsilon = epsilon, 
                                                       Cx = Cx, eta = eta, kernel = 'logistic')})[3]
        out_par[[10]] = rbind(out_par[[10]], tmp)
        
        out_time[t, 11] = system.time({tmp = sgd_convsm(theta = c(0, 0, 0), x = x, y = y, 
                                                       tau = tau, h = 0.5, epsilon = epsilon, 
                                                       Cx = Cx, eta = eta, kernel = 'uniform')})[3]
        out_par[[11]] = rbind(out_par[[11]], tmp)
        
        out_time[t, 12] = system.time({tmp = sgd_convsm(theta = c(0, 0, 0), x = x, y = y, 
                                                       tau = tau, h = 3, epsilon = epsilon, 
                                                       Cx = Cx, eta = eta, kernel = 'uniform')})[3]
...
oper = foreach(i=start:end, .combine=rbind, .multicombine=TRUE, #change
               .init=list()) %dopar% {
                   ...
                   x = cbind(1, x)
                   theta = matrix(c(3, 2, 0.5))
                   y = x%*% theta + rexp(2500, 0.1)
                   
                   out = dp_qr3(x = x, y = y, theta = theta, epsilon = 1,
                               taus = c(0.05, 0.25, 0.5, 0.75, 0.95, 0.99)) #change
               }

stopCluster(workers)

save(oper, file = paste0("./output/simulation_expnoise/opm_sgd_n2500_eps1_", iter, ".Rdata"))
