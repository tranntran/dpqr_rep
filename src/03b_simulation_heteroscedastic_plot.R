...
plot_best_methods = function(n, eps) {
    out = rep(list(NULL), 25)
    
    if (n == 2500) {
        for (k in 1:20) {
            file_name = paste0("output/simulation_heteroscedastic/kng_n", n, "_eps", eps, "_", k, ".Rdata")
            load(file_name)
            for (i in 1:25) {
                tmp = t(oper[[i]])
                for (j in 1:6) {
        ...
        for (k in c(1:6, 10:20)) {
            file_name = paste0("output/simulation_heteroscedastic/opm_sgd_n", n, "_eps", eps, "_", k, ".Rdata")
            load(file_name)
            for (i in 1:25) {
                tmp = t(oper[[i]])
                for (j in 1:12) {
        ...
        for (k in 1:20) {
            file_name = paste0("output/simulation_heteroscedastic/agd_np_n", n, "_eps", eps, "_", k, ".Rdata")
            load(file_name)
            for (i in 1:25) {
                tmp = t(oper[[i]])
                for (j in 1:7) {
                    out[[j+18]] = rbind(out[[j+18]], tmp[j,])
                }
            }
        }
    } else if (n == 250) {
    ...
    tab_best_best = tab[c(3, 9, 15, 21, 25),]
    ...
    pl = ggplot(tab_best_best, aes(x = Quantile, y = Value, group = Method)) +
        ...
        coord_cartesian(ylim=c(-2.75, yl)) + 
...
time_best_methods = function(n, eps) {
    ...
    tab_best_best$Method  = factor(tab_best_best$Method, 
                                   levels = c("Non-Private", 'KNG Logistic (h=0.5)',
                                              'OPM Logistic (h=0.5)', 'DPSGD Logistic (h=0.5)',
                                              'DPAGD Logistic (h=0.5)'))
    tab_se_best_best = tab_se[c(3, 9, 15, 21, 25),]
...
save_file = paste0('./plot/', save_date, '_compare_all_hetero.png')
...
plot_all_methods = function(method, n, eps) {
    ...
    out = rep(list(NULL), 7)
    k_max = ifelse(n == 2500, 20, 100)
    i_max = ifelse(n == 2500, 25, 5)
    if (method == 'kng') {
        for (k in 1:k_max) {
            file_name = paste0("output/simulation_heteroscedastic/kng_n", n, "_eps", eps, "_", k, ".Rdata")
            load(file_name)
    ...
    if (method == 'kng') {ylim = c(-2, 2)} 
    else if (method == 'opm') {ylim = c(-3, 2)}
    else if (method == 'sgd') {ylim = c(-2, 3)}
    else if (method == 'agd') {ylim = c(-2, 3)}
...
for (method in methods) {
    ...
    save_date = paste0(substring(save_date, 3, 4), substring(save_date, 6, 7),
                       substring(save_date, 9, 10))
    save_file = paste0('./plot/', save_date, '_compare_all_', method, '_hetero.png')
