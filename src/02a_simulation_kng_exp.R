...
# check loss with regularization. The other two file has name simulation_[method]_exp.R.
...
oper = foreach(i=start:end, .combine=rbind, .multicombine=TRUE, #change
               .init=list()) %dopar% {
                   ...
                   out = dp_qr2(x = x, y = y, theta = theta, epsilon = 1,
                               taus = c(0.05, 0.25, 0.5, 0.75, 0.95, 0.99)) #change
               }

stopCluster(workers)

save(oper, file = paste0("./output/simulation_expnoise/kng_n2500_eps1_", iter, ".Rdata"))
