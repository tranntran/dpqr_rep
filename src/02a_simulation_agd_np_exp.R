...
dp_qr = function(x, y, theta, taus, epsilon, delta = 1e-8) { #nsteps = 100000, Cx = 1) {
  out_diff = NULL
...
oper = foreach(i=start:end, .combine=rbind, .multicombine=TRUE, #change
               .init=list()) %dopar% {
                 ...
                 out = dp_qr(x = x, y = y, theta = theta, epsilon = 1,
                             taus = c(0.05, 0.25, 0.5, 0.75, 0.95, 0.99)) #change
               }

stopCluster(workers)

save(oper, file = paste0("./output/simulation_expnoise/agd_np_n2500_eps1_", iter, ".Rdata"))
