...
# in the simulation setting with heteroscedastic data. For each replication, we 
...
# check loss with regularization. The other two file has name 
# simulation_[method]_heteroscedastic.R.
...
dp_qr2 = function(x, y, theta, taus, epsilon, delta = 1e-8, nsteps = 100000, Cx = 1) {
    ...
    for (t in 1:length(taus)) {
        ...
        start_points = matrix(0, ncol = ncol(x), nrow = 4)
        ...
        out_time[t, 7] = system.time({tmp = MCMC(start_points, kng_qr_reg, nsteps = nsteps, 
                                                 nchains = 4, x = x, y = y, tau = tau, Cx = Cx,
                                                 epsilon = epsilon, conv_checker = convergence_gelman(), 
                                                 kernel = kernel_ram())})[3]
        out_par[[7]] = rbind(out_par[[7]], tail(as.matrix(tmp[[1]]), 1))
...
oper = foreach(i=start:end, .combine=rbind, .multicombine=TRUE, #change
               .init=list()) %dopar% {
                   ...
                   out = dp_qr2(x = x, y = y, theta = theta, epsilon = 1, Cx = 2,
                                taus = c(0.05, 0.25, 0.5, 0.75, 0.95, 0.99)) #change
               }

stopCluster(workers)

save(oper, file = paste0("./output/simulation_heteroscedastic/kng_n2500_eps1_", iter, ".Rdata"))
